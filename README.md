# 👋 Hi, I'm Sivateja

### 🛡️ Cybersecurity Student | Cloud Security | SOC & Threat Detection

**B.Tech Computer Science Engineering — Cybersecurity**
**KL University**

I’m a cybersecurity-focused developer interested in **threat detection, cloud security, network security, SOC operations, and automotive cybersecurity**. I enjoy building practical security projects that combine programming, networking, cloud technologies, and security engineering.

<p align="center">
  <a href="https://github.com/sivateja7285">
    <img src="https://img.shields.io/badge/GitHub-sivateja7285-181717?style=for-the-badge&logo=github&logoColor=white" />
  </a>
  <a href="https://www.linkedin.com/">
    <img src="https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
  <a href="mailto:2400030961@kluniversity.in">
    <img src="https://img.shields.io/badge/Email-Contact-EA4335?style=for-the-badge&logo=gmail&logoColor=white" />
  </a>
</p>

---

## 🔐 What I Work On

* 🛡️ **Cybersecurity & Threat Detection**
* ☁️ **Cloud Security & AWS**
* 🚗 **Automotive Cybersecurity**
* 🌐 **Network Security**
* 🔎 **SOC & SIEM Concepts**
* 🐧 **Linux & Security Tools**
* 🐍 **Python Security Automation**
* 🎯 **Threat Intelligence & MITRE ATT&CK**

---

## 🚀 Featured Projects

### 🚗 Automotive Cybersecurity SOC Dashboard

A real-time automotive cybersecurity monitoring platform designed to simulate and detect attacks on **CAN bus communication**.

**Key Features:**

* Real-time CAN traffic simulation
* Multi-ECU simulation
* DoS attack detection
* Replay attack detection
* Speed spoofing detection
* RPM manipulation detection
* Threat severity classification
* MITRE ATT&CK technique mapping
* SOC-style Streamlit dashboard
* Real-time vehicle telemetry monitoring

**Tech Stack:**
`Python` `Streamlit` `CAN Bus` `Cybersecurity` `MITRE ATT&CK`

🔗 **Repository:**
[https://github.com/sivateja7285/Automotive-Cybersecurity-SOC-Dashboard](https://github.com/sivateja7285/Automotive-Cybersecurity-SOC-Dashboard)

---

### 🎣 Phishing Detection & URL Analyzer

A cybersecurity project focused on analyzing URLs and identifying indicators associated with potentially malicious or phishing websites.

**Focus Areas:**

* URL analysis
* Suspicious domain detection
* Phishing indicators
* Security-focused automation
* Threat classification

**Tech Stack:**
`Python` `Cybersecurity` `Machine Learning` `URL Analysis`

---

### ☁️ Cloud Security Monitoring Lab

A practical AWS security environment designed to explore cloud infrastructure security, identity management, monitoring, logging, and threat detection.

**Focus Areas:**

* AWS IAM
* EC2 security
* VPC security
* Security Groups
* Cloud monitoring
* Logging
* Security detection
* Cloud security best practices

**Tech Stack:**
`AWS` `EC2` `IAM` `VPC` `Cloud Security` `Linux`

---

## 🧰 Tech Stack

### 💻 Programming

<p>
  <img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white"/>
  <img src="https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white"/>
  <img src="https://img.shields.io/badge/SQL-4479A1?style=for-the-badge&logo=postgresql&logoColor=white"/>
</p>

### ☁️ Cloud & Infrastructure

<p>
  <img src="https://img.shields.io/badge/AWS-232F3E?style=for-the-badge&logo=amazonaws&logoColor=white"/>
  <img src="https://img.shields.io/badge/EC2-FF9900?style=for-the-badge&logo=amazonec2&logoColor=white"/>
  <img src="https://img.shields.io/badge/IAM-FF9900?style=for-the-badge&logo=amazonaws&logoColor=white"/>
  <img src="https://img.shields.io/badge/VPC-FF9900?style=for-the-badge&logo=amazonaws&logoColor=white"/>
</p>

### 🛡️ Cybersecurity

<p>
  <img src="https://img.shields.io/badge/Wireshark-1679A7?style=for-the-badge&logo=wireshark&logoColor=white"/>
  <img src="https://img.shields.io/badge/Nmap-4682B4?style=for-the-badge"/>
  <img src="https://img.shields.io/badge/MITRE%20ATT%26CK-EF3B2D?style=for-the-badge"/>
  <img src="https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black"/>
</p>

### 🔧 Development & Tools

<p>
  <img src="https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white"/>
  <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white"/>
  <img src="https://img.shields.io/badge/VS%20Code-007ACC?style=for-the-badge&logo=visualstudiocode&logoColor=white"/>
  <img src="https://img.shields.io/badge/Streamlit-FF4B4B?style=for-the-badge&logo=streamlit&logoColor=white"/>
</p>

---

## 🎯 Cybersecurity Interests

```text
Cybersecurity
│
├── 🛡️ Threat Detection
│   ├── SIEM
│   ├── SOC Operations
│   ├── Threat Hunting
│   └── Incident Detection
│
├── ☁️ Cloud Security
│   ├── AWS IAM
│   ├── EC2
│   ├── VPC
│   └── Cloud Monitoring
│
├── 🌐 Network Security
│   ├── Network Analysis
│   ├── Wireshark
│   ├── Nmap
│   └── Protocol Security
│
└── 🚗 Automotive Cybersecurity
    ├── CAN Bus Security
    ├── ECU Monitoring
    ├── DoS Detection
    ├── Replay Detection
    └── MITRE ATT&CK
```

---

## 📜 Certifications & Experience

### ☁️ AWS Certified Cloud Practitioner

Foundational certification covering AWS Cloud concepts, architecture, security, services, and cloud economics.

### 🛡️ Palo Alto Networks Internship

Completed cybersecurity-focused internship experience involving security concepts and practical learning.

---

## 📚 Currently Learning

* 🔎 Security Operations & SOC Engineering
* 🧠 Machine Learning for Cybersecurity
* ☁️ Advanced AWS Cloud Security
* 🚗 Automotive & CAN Bus Security
* 🎯 Threat Intelligence
* 🛡️ SIEM & Detection Engineering
* 🐍 Python Security Automation
* 🔬 Security Research

---

## 📊 GitHub Statistics

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=sivateja7285&show_icons=true&theme=tokyonight&hide_border=true&count_private=true" height="170"/>
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=sivateja7285&layout=compact&theme=tokyonight&hide_border=true" height="170"/>
</p>

---

## 🔥 Contribution Streak

<p align="center">
  <img src="https://streak-stats.demolab.com?user=sivateja7285&theme=tokyonight&hide_border=true" />
</p>

---

## 🐍 Contribution Activity

<p align="center">
  <img src="https://raw.githubusercontent.com/sivateja7285/sivateja7285/output/github-contribution-grid-snake-dark.svg" />
</p>

---

## 📈 GitHub Contribution Graph

<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=sivateja7285&theme=tokyo-night&hide_border=true" width="95%"/>
</p>

---

## 💡 My Goal

> **Build practical cybersecurity solutions that connect cloud security, threat detection, networking, and real-world security engineering.**

I’m continuously working on projects that help me move from **learning cybersecurity concepts → implementing them → building security tools → solving real-world security problems.**

---

## 🤝 Let's Connect

<p align="center">
  <a href="https://github.com/sivateja7285">
    <img src="https://img.shields.io/badge/GitHub-Follow-181717?style=for-the-badge&logo=github"/>
  </a>
  <a href="mailto:2400030961@kluniversity.in">
    <img src="https://img.shields.io/badge/Email-Contact-EA4335?style=for-the-badge&logo=gmail&logoColor=white"/>
  </a>
</p>

<p align="center">
  <b>🛡️ Cybersecurity • ☁️ Cloud Security • 🚗 Automotive Security • 🔎 Threat Detection</b>
</p>
